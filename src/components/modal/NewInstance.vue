    ...
    <v-dialog v-model="formDialog" max-width="800px" transition="dialog-transition">
      <v-card>
        <v-toolbar color="primary" dark>
          ...
          <v-toolbar-title>Add Instance</v-toolbar-title>
        </v-toolbar>
        ...
        <v-card-actions>
          ...
          <v-spacer/>
          <v-btn color="primary" @click="debugSubmit" :disabled="step !== 3">Submit</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  methods: {
    ...
    debugSubmit() {
      ...
      const paymentRange = [];
      paymentRange.push(parseInt(this.instance.paymentStart));
      paymentRange.push(parseInt(this.instance.paymentEnd));
  ...
  created() {
    window.getApp.$on("Instance", () => {
      ...
      const ownerData = this.$store.state.owner;
    });
